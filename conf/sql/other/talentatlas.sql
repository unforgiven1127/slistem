...
CREATE TABLE IF NOT EXISTS `job_mailfriend` (
  `job_mailfriendpk` int(11) NOT NULL AUTO_INCREMENT,
  `sender_name` varchar(255) COLLATE utf8_bin NOT NULL,
  `sender_email` varchar(255) COLLATE utf8_bin NOT NULL,
  `receiver_email1` varchar(255) COLLATE utf8_bin NOT NULL,
  `receiver_email2` varchar(255) COLLATE utf8_bin NOT NULL,
  `receiver_email3` varchar(255) COLLATE utf8_bin NOT NULL,
  `message` text COLLATE utf8_bin NOT NULL,
  `positionfk` int(11) NOT NULL,
  PRIMARY KEY (`job_mailfriendpk`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COLLATE=utf8_bin AUTO_INCREMENT=1 ;


ALTER TABLE `position` ADD `company_label` VARCHAR(255) NULL ;
ALTER TABLE `position` ADD `to_jobboard` integer NULL ;
ALTER TABLE `position` ADD `external_key` integer NULL ;
CREATE INDEX idx_external_key ON `position` (external_key) USING BTREE;

ALTER TABLE `position` ADD `expiration_date` TIMESTAMP NULL;
CREATE INDEX idx_expiration_date ON `position` (expiration_date);



-- Stef: 05-february
-- Adding missing right in the jobboard user section

INSERT INTO `right` (`label`, `description`, `type`, `cp_uid`, `cp_action`, `cp_type`, `cp_pk`) VALUES
('Save the created position', 'Access right to save the created position', 'alias', '654-321', 'ppase', 'ppaj', 0);