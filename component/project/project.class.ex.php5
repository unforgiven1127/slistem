...
class CProjectEx extends CProject
{
  ...
  public function getPageActions($psAction = '', $psType = '', $pnPk = 0)
  {
    ...
    $nUserPk = $oLogin->getUserPk();
    if(!empty($pnPk))
      $bEdit = $this->_canEdit($pnPk, $nUserPk);
  ...
  public function getSearchResult($psDatatype, $psKeywords, $psFieldType = 'text', $pnDisplayPage=0)
  {
    ...
    switch ($psFieldType)
    {
      case 'text':
        ...
        $sWhere = '';
        foreach ($aSearchFields[$psDatatype]['fields']['text'] as $sField)
        {
          if($nCount>0)
            $sWhere .= ' OR ';
    ...
    public function getSearchResultMeta($psType = '')
    {
      $aResultMeta =
      array(
          CONST_PROJECT_TYPE_PROJECT =>
          array(
              'pk' => 'projectpk',
              'title' => '%title%',
              'excerpt' => '%description%',
              'more-data' => 'Deadline : %date_end%'
          )
      );

      if(!empty($aResultMeta[$psType]))
        return $aResultMeta[$psType];
    ...
    {
      $aFields = array(
            CONST_PROJECT_TYPE_PROJECT => array(
                'table' => 'project',
                'label' => 'Projects',
                'fields' => array (
                    'text' => array('title', 'description'),
                    'date' => array('date_end')
                    )
                )
          );

      if(!empty($aFields[$psType]))
        return $aFields[$psType];
  ...
  private function _getProjectList()
  {
    ...
    $oPage->addCssFile($this->getResourcePath().'css/project.css');
    $oPage->addCssFile($this->getResourcePath().'css/projectb.css');
    $sHTML = $oHTML->getTitleLine('Project list', $this->getResourcePath().'pictures/project_48.png');

    $asTabs = array();
    $asTabs[] = array('title' => 'In Progress', 'label' => 'inprogress', 'content' => $this->_getProjectResultList(false,'p.date_start<\''.date('Y-m-d h:i:s').'\''));
    $asTabs[] = array('title' => 'Coming next', 'label' => 'coming', 'content' => $this->_getProjectResultList(false,'p.date_start>\''.date('Y-m-d h:i:s').'\''));
    $asTabs[] = array('title' => 'Job Done', 'label' => 'jobdone', 'content' => $this->_getProjectResultList(true));

    $sHTML.= $oHTML->getBlocStart('', array('class' =>'bottom_container'));
    $sHTML.= $oHTML->getTabs('projectlist_tabs', $asTabs);
    $sHTML.= $oHTML->getBlocEnd();
    $sHTML.= $oHTML->getCR(2);
  ...
  private function _getProjects($pbCurrentProject, $psParam = '')
  {
    $oDB = CDependency::getComponentByName('database');
    $oLogin = CDependency::getCpLogin();
   ...
   * @return string
   */
  private function _getProject($pbCurrentProject, $psParam= '')
  {
    $oHTML = CDependency::getCpHtml();
    $oPage = CDependency::getCpPage();
    $oPager = CDependency::getComponentByName('pager');
    ...
    $asProjectData = $this->_getProjects($pbCurrentProject, $psParam);
  ...
  private function _getProjectListRow($pasProjectData, $pasUserData)
  {
    ...
    $sHTML = $oHTML->getBlocStart('', array('class' => 'divlist-item columns'));

      $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:64px; height:64px; padding:10px 20px 10px 20px;'));
      if($pasProjectData['finishing'] && !$pasProjectData['finished'])
      $sHTML.= $oHTML->getPicture(CONST_PICTURE_IMPORTANT, 'Attention required');
      if($pasProjectData['finished'])
      $sHTML.= $oHTML->getPicture(CONST_PICTURE_COMPLETE, 'Project completed');
      $sHTML.= $oHTML->getBlocEnd();

      $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:80%; margin-bottom:15px;'));

        $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:100%;', 'class' => 'item-title'));
        $sHTML.= $oHTML->getLink($pasProjectData['title'], $sUrl, array('title' => $pasProjectData['description']));
        $sHTML.= $oHTML->getBlocEnd();

        $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:100%;', 'class' => 'item-description'));
        $sHTML.= $pasProjectData['description'];
        $sHTML.= $oHTML->getBlocEnd();

      $sHTML.= $oHTML->getBlocEnd();

      $sHTML.= $oHTML->getBlocStart('', array('class' => 'columns'));

        // Date and creator
        $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:180px; min-height:40px; display:block;'));
        $sHTML.= 'Created on '.getFormatedDate('Y-m-d', $pasProjectData['date_create']);
        $sHTML.= $oHTML->getCR();
        if(!empty($pasUserData[$pasProjectData['creatorfk']]))
          $sHTML.= $oHTML->getText('by '.$oLogin->getUserNameFromData($pasUserData[$pasProjectData['creatorfk']]));
        $sHTML.= $oHTML->getBlocEnd();

        // Deadline and percentage
        $nDiff = floor((time()-date(strtotime($pasProjectData['date_end'])))/(60*60*24));
        $sCountDown= ($nDiff>-1 ? '<span class=\'red\'>D+'.$nDiff.'</span>'.$oHTML->getCR() : '<span>D'.$nDiff.'</span>'.$oHTML->getCR());

        $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:130px; margin-bottom:10px;', 'class' => 'deadline'));
        $sHTML.= $sCountDown;
        $sHTML.= getFormatedDate('Y-m-d', $pasProjectData['date_end']);
        $sHTML.= $oHTML->getBlocEnd();

        // Percentage done
        $sClass = $this->_getProgressClass($pasProjectData['progress']);
        $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:250px; margin-bottom:10px;'));
        $sHTML.= $oHTML->getBloc('pg_'.$pasProjectData['projectpk'], '', array('class' => 'progressbar '.$sClass));
        $sHTML.= $pasProjectData['progress'].'% done';
        $sHTML.= $oHTML->getBlocEnd();
        $oPage->addCssFile('http://code.jquery.com/ui/1.10.1/themes/base/jquery-ui.css');
        $oPage->addJsFile(CONST_PATH_JS_JQUERYUI);
        $sHTML.= '<script>$( "#pg_'.$pasProjectData['projectpk'].'" ).progressbar({ value: '.$pasProjectData['progress'].' });</script>';

      $sHTML.= $oHTML->getBlocEnd();

        $sHTML.= $oHTML->getBlocStart('', array('style' => 'float:right; width:130px; margin-top:-40px;', 'class' => 'button-list'));
  ...
  {
    // !!!!! TODO: !!!!!!!  called for every displayed row, it shouldn't make queries everytime
    if(!assert('is_integer($pnProjectPk)'))
      return false;

    //TODO: check how independent tasks are managed, we should make it possible
    if(empty($pnProjectPk))
      return false;
  ...
  private function _getProjectView($pnPK)
  {
    ...
    $oPage->addCssFile(array($this->getResourcePath().'css/projectb.css'));
    ...
    $sHTML.= $oHTML->getBlocStart('', array('class' => 'columns', 'style' => 'padding:10px;'));

      $nPorjectPk = (int)$asProjectData['projectpk'];
      $asUser = $oLogin->getUserList(0,false,true);

      // Created by X on Y
      $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:250px;'));
      $sHTML.= $oHTML->getText('Created on ');
      $sHTML.= getFormatedDate('Y-m-d',$asProjectData['date_create']).$oHTML->getCR();
      ...
      // Deadline
      $nDiff = floor((time()-date(strtotime($asProjectData['date_end'])))/(60*60*24));
      $sCountDown= ($nDiff>-1 ? '<span class=\'red\'>D+'.$nDiff.'</span>'.$oHTML->getCR() : '<span>D'.$nDiff.'</span>'.$oHTML->getCR());

      $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:180px; margin-bottom:10px;', 'class' => 'deadline'));
      $sHTML.= $sCountDown;
      $sHTML.= getFormatedDate('Y-m-d', $asProjectData['date_end']);
      $sHTML.= $oHTML->getBlocEnd();

      // Percentage done
      // TO DO : Fix bug on the progress bar
      $sClass = $this->_getProgressClass($asProjectData['progress']);
      $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:250px; margin-bottom:10px;'));
      $sHTML.= $oHTML->getBloc('pg', '', array('class' => 'progressbar '.$sClass));
      $sHTML.= $asProjectData['progress'].'% done';
      $sHTML.= $oHTML->getBlocEnd();
      $oPage->addCssFile('http://code.jquery.com/ui/1.10.1/themes/base/jquery-ui.css');
      $oPage->addJsFile(CONST_PATH_JS_JQUERYUI);
      $sHTML.= '<script> $("#pg").progressbar({ value:'.$asProjectData['progress'].' }); </script>';

      $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:200px; margin-bottom:10px; margin-left:50px;'));
      $sHTML.= $oHTML->getText('In charge of the project :').$oHTML->getCR();
    ...
    $asTabs[] = array('label' => 'tasks', 'title' => 'Tasks', 'content' => $this->_getTasksTab((int)$asProjectData['projectpk']));
    $asTabs[] = array('label' => 'documents', 'title' => 'Documents', 'content' => 'Coming next');

    $sHTML.= $oHTML->getTabs('project_tabs', $asTabs);

    return $sHTML;
  }

  function _getTasksTab($pnPK) {
    ...
    $sURL = $oPage->getUrl('project', CONST_ACTION_ADD, CONST_PROJECT_TYPE_TASK, 0, array('prjpk'=>$pnPK));
    ...
    else
    {
      ...
      $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:250px; margin-bottom:10px;'));
      $sHTML.= $oHTML->getBloc('tasks_'.$pnPK, '', array('class' => 'progressbar '.$sClass));
      $sHTML.= $nTotal.'% done';
      $sHTML.= $oHTML->getBlocEnd();
      $oPage->addCssFile('http://code.jquery.com/ui/1.10.1/themes/base/jquery-ui.css');
      $oPage->addJsFile(CONST_PATH_JS_JQUERYUI);
      $sHTML.= '<script> $("#tasks_'.$pnPK.'").progressbar({ value:'.$nTotal.' }); </script>';

      $asTabs = array();
      $asTabs[] = array('label' => 'all', 'title' => 'All('.array_sum($anTaskByType).'/'.array_sum($anDoneByType).')', 'content'=>$this->_getTabs('',$pnPK,$asUser));
      foreach($this->casTaskType as $sType)
      {
        ...
        if(($anTaskByType[$sType] + $anDoneByType[$sType]) > 0)
         $asTabs[] = array('label' => $sType, 'title' => $sType.' ('.$anTaskByType[$sType].'/'.$anDoneByType[$sType].')','content'=>$this->_getTabs($sType,$pnPK,$asUser));
      }

      $sHTML.= $oHTML->getTabs('tasks_tabs', $asTabs, 'all', 'vertical');
  ...
  private function _getTabs($psType='',$pnPK,$pasUser)
  {
    ...
    if($psType!='')
     {
      $sHTML.= $oHTML->getBlocStart($psType.'Id', array('class' => 'divlist'));
        $nLineNb = 1;
        $nCount = 0;
          while($bRead)
          {
            ...
            if($asTask['type'] == $psType)
            {
              ...
              if($asTask['status'])
               ...
               $sHTML.= $oHTML->getBlocStart('', array('class' => 'divlist-item projectView_taskrow '.$sExtraClass, 'style' => 'border-left: 10px solid; border-left-color:'.$this->casTaskTypeBgColor[$psType].';'));
               ...
               $sjavascript = "
                ...
                if($bEdit)
                {
                  ...
                  $sHTML.= $oHTML->getLink($sPic, 'javascript:;', array('onclick' => 'goPopup.setLayerFromAjax(null, \''.$sURL.'\', true);'));
          ...
          {
            $sHTML.= $oHTML->getBlocStart('allId', array('class' => 'divlist'));
            $nLineNb = 1;
            $nCount = 0;
            while($bRead)
            {
                ...
                $sHTML.= $oHTML->getBlocStart('', array('class' => 'divlist-item projectView_taskrow '.$sExtraClass));
                $asTaskActor = explode(',', $asTask['actors']);
  ...
  private function _getProjectGraphicalView($pnPK)
  {
   ...
   //start displaying the project's tasks
    ...
    $oPage->addJsFile(CONST_PATH_JS_DRAGDROP);
    $oPage->addJsFile($this->getResourcePath().'js/project.js');
  ...
  private function _getProjectResultList($psStatus, $psParam = '')
  {

    //check if it is not empty
     $oHTML = CDependency::getCpHtml();

      $sHTML = $oHTML->getBlocStart('cprojectContainer_'.$psStatus, array('class' => 'divlist'));
      $sHTML.= $this->_getProject((boolean)$psStatus, $psParam);
      $sHTML.= $oHTML->getBlocEnd();
  ...
  */
  ...
  private function _getTaskList($pbMyTask = true, $pbManagingTask = false)
  {
    ...
    if(empty($sOrder) || $sOrder == 'date')
    {
     ...
     $asTabs = array();
     $asTabs[] = array('label' => 'todolist', 'title' => 'My todo list', 'content' => $this->_getTasks(true, false));
     $asTabs[] = array('label' => 'followedtasks', 'title' => 'Tasks I\'m following', 'content' => $this->_getTasks(false, true));

     $sHTML.= $oHTML->getBlocStart('', array('class' =>'bottom_container'));
     $sHTML.= $oHTML->getTabs('mytasks_tab', $asTabs);
     $sHTML.= $oHTML->getBlocEnd();
     $sHTML.= $oHTML->getCR(2);
  ...
  private function _getTaskListRow($pasTaskData, $pnUserPk, $pbUsersTask = true, $pbDisplayProject = true)
  {
   ...
   $sAccess = $oRight->canAccess($this->_getUid(),CONST_ACTION_DELETE,$this->getType(),$this->getPk());
    ...
    $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:185px; float:left;  padding: 0 5px; line-height: 18px; '));
    $sHTML.= $oHTML->getText('Created : ');
    $sHTML.= $oHTML->getNiceTime($pasTaskData['date_start'], 0, true);
    $sHTML.= $oHTML->getCR();
    $sHTML.= $oHTML->getText('By : ');
    if($pasTaskData['creatorfk'] == $oLogin->getUserPk())
      $sHTML.= $oHTML->getText('me');
    else
      $sHTML.= $oHTML->getText($pasTaskData['firstname']);
    $sHTML.= $oHTML->getCR();

    if(!empty($pasTaskData['date_end']))
    {
    ...
    $sHTML.= $oHTML->getBlocStart('', array('class' => 'taskRowProgress', 'style' => 'width:50px; float:left; text-align: center;'));
    ...
    if(empty($pasTaskData['progress']))
      $pasTaskData['progress'] = ' 0 ';
    ...
    $sHTML.= $oHTML->getBlocStart('', array('style' => 'width:430px; float:left;  padding: 0 5px;'));
      $sHTML.= $oHTML->getBlocStart('', array('class' => 'projectTaskTitle'));
      ...
      if(!empty($pasTaskData['description']))
      {
       $sHTML.= $oHTML->getBlocStart('', array('class' => 'projectTaskDesc'));

       if(strlen($pasTaskData['description']) > 75)
       {
         $sHTML.= $oHTML->getText(substr($pasTaskData['description'], 0, 75));
         $sHTML.= $oHTML->getCR();
         if(strlen($pasTaskData['description']) > 150)
           $sHTML.= $oHTML->getText(substr($pasTaskData['description'], 75, 145).'... ');
         else
           $sHTML.= $oHTML->getText(substr($pasTaskData['description'], 75).'... ');
       }
       else
         $sHTML.= $oHTML->getText($pasTaskData['description']);
  ...
  private function _getMagazineView($pnPK)
  {
    ...
    $oPage->addJsFile($this->getResourcePath().'js/ad-gallery.js');
    $oPage->addCustomJs($sJavascript);
