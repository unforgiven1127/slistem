...
class CWebMailEx extends CWebMail
{
  ...
  {
    if(!assert('is_integer($psMailPk)'))
      return array('error' => 'Bad parameters.');

    if(empty($psMailPk))
    {
      $psMailPk = (int)  getValue('loginpk');
    }
  ...
  private function _getWebMailForm($pnMailPk, $psItemType = '', $pnItemPk = 0)
  {
    ...
    $sHtmlBtn.= '<span><a href="javascript:;" onclick="goPopup.removeByType(\'layer\');"><img src='.$this->getResourcePath().'/pictures/close_button.jpg'.' width="32" height="32"></a></span>';
    ...
    $oForm->addField('misc', '', array('type' => 'text', 'class' => 'formLink', 'text' => 'Add recipients', 'onclick' => '$(\'.hiddenField\').fadeToggle(\'fast\');', 'style' => 'cursor: pointer;'));

    $oForm->addField('input', 'receiver_cc', array('label' => 'cc', 'value' => '', 'style' => 'width:100%;'));
    $oForm->setFieldControl('receiver_cc', array('jsFieldTypeEmail' => ''));
    $oForm->setFieldDisplayparams('receiver_cc', array('class' => 'hiddenField hidden'));
  ...
  private function _sendWebMail($pnMailPk)
  {
    ...
    return array('notice' => 'Your mail has been sent successfully.', 'action' => 'goPopup.removeLastByType(\'layer\');');
  }
