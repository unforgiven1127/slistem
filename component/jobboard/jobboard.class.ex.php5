...
class CJobboardEx extends CJobboard
{
  ...
  private function _getMailFriendForm()
  {
    ...
    $oForm->setFormParams('', true, array('submitLabel' => 'Send','action' => $sURL, /*'onsubmit' => 'event.preventDefault(); '.$sJs.' return false; ',*/ 'noCancelButton' => 1, 'noCloseButton' => 1));
  ...
  private function _getMailFriendSave()
  {
   ...
   if(empty($sSenderName))
    ...
    $sQuery = 'SELECT pos.*,cp.company_name,ind.name AS industry_name FROM position AS pos  ';
    $sQuery.= 'LEFT JOIN addressbook_company AS cp ON (cp.addressbook_companypk = pos.companyfk) ';
    $sQuery.= 'LEFT JOIN addressbook_industry AS ind ON (pos.industryfk = ind.addressbook_industrypk) ';
    $sQuery.= 'WHERE pos.visibility <> 0 and pos.positionpk = '.$nPositionPk;
    ...
    //save before in casde it crashes when sending emails
  ...
  private function _getJobResultList($pavResult, $psSearchId = '', $pbSearchFormOpen = false)
  {
    ...
    $oPage = CDependency::getComponentByName('page');
    $oPager = CDependency::getComponentByName('pager');
    $oPage->addJsFile($this->getResourcePath().'js/jobboard.js');
  ...
  private function _getSqlJobSearch()
  {
    ...
    if(!$bGlobalSearch)
    {
      ...
      if(!empty($sIndustry))
      {
        ...
        $asWhereSql[] = ' (ind.addressbook_industrypk IN ('.$sIndustry.') OR ind.parentfk IN ('.$sIndustry.')) ';
      }
  ...
  */
  private function _getJobSearchForm($pbIsAjaxForm = false)
  {
    ...
    {
      if(in_array($asData['addressbook_industrypk'], $asIndustrySelected))
        $oForm->addOption('industry_tree', array('title' => ''.$this->casText[$asData['name']].'','id' => $asData['addressbook_industrypk'], 'parent' => $asData['parentfk'],'checked'=>'checked'));
      else
        $oForm->addOption('industry_tree', array('title' => ''.$this->casText[$asData['name']].'','id' => $asData['addressbook_industrypk'], 'parent' => $asData['parentfk']));
    }
    ...
    if(!empty($asData))
    {
      $sHTML.= $oHTML->getBlocStart('',array('class'=>'simpleRedBorderTop', 'style' => 'margin-right: 10px;'));
        ...
        $sHTML.= $oHTML->getBlocStart('');
          ...
          $sAjax = $oHTML->getAjaxPopupJS($sURL, 'body','','600','500',1);
  ...
  */
  private function _getJobDetailInformation($pnPk)
  {
    ...
    $sQuery.= ' LEFT JOIN addressbook_company AS cp ON (cp.addressbook_companypk = pos.companyfk) ';
    $sQuery.= ' LEFT JOIN addressbook_industry AS ind ON (pos.industryfk = ind.addressbook_industrypk) ';
    $sQuery.= ' LEFT JOIN addressbook_industry AS ind_parent ON (ind_parent.addressbook_industrypk = ind.parentfk) ';
  ...
  */
  private function _getResumeSave($pnPositionPk)
  {
    ...
    //Notify user we received his application
    $oMail->sendRawEmail(CONST_CRM_MAIL_SENDER, $sEmail, $sSubject, $sContent);
  ...
  */
  public function getCompanies($pnCompanyPk = 0)
  {
    ...
    {
      $sQuery = 'SELECT * FROM addressbook_company WHERE status = 1 ORDER BY company_name ASC';
      $oResult = $oDB->ExecuteQuery($sQuery);
      $bRead = $oResult->readFirst();

      while($bRead)
      {
        $asCompanies[$oResult->getFieldValue('addressbook_companypk')] = $oResult->getData();
        $bRead = $oResult->readNext();
      }
    ...
    $sQuery = 'SELECT * FROM addressbook_company WHERE addressbook_companypk = '.$pnCompanyPk;
    $oResult = $oDB->ExecuteQuery($sQuery);
    $bRead = $oResult->readFirst();
  ...
  */
  ...
  private function _getJobSlider($psSearchId)
  {
    ...
    $oPage->addJsFile($this->getResourcePath().'/js/jquery.totemticker.js');
  ...
  private function _getConsultantPictureByIndustry($pnIndustryPk, $pbLinkOnly = false)
  {
    ...
    if(empty($this->casConsultantIndustries))
    {
      ...
      {
        if(!empty($asIndustry['addressbook_industrypk']))
        {
          $nIndustryPk = (int)$asIndustry['addressbook_industrypk'];
  ...
  private function _getRssDescription()
  {
    ...
    foreach($asIndustry as $asIndusData)
    {
      if(isset($this->casText[$asIndusData['name']]))
      {
        ...
        $oIndustry->addAttribute('id', $asIndusData['addressbook_industrypk']);
      }
    }
  ...
  private function _getRssFeed()
  {
    ...
    $sQuery.= ' INNER JOIN addressbook_industry as ind on (ind.addressbook_industrypk = pos.industryfk) ';
    $sQuery.= ' LEFT JOIN addressbook_industry as parent_ind on (parent_ind.addressbook_industrypk = ind.parentfk) ';
