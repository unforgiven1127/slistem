...
class CGbTestEx extends CGbTest
{
  ...
  public function getCronJob()
  {
    ...
    //all the emails are fired at 6am
    if($time == 6 || (getValue('custom_uid') == $this->csUid && $bForceCron))
    {
      $nCount = 0;

      $sLog = '<h3>6AM, time for notifications</h3><br /><br />Notify Participants deadlines...<br />';
      $nCount = $this->_notifyStudents();
      $sLog.=  $nCount.' participants were notified the deadline is approching.<br /><br />';

      $sLog.= 'Notify Participants ESA2 available...<br />';
      $nCount = $this->_notifyEsa2Available();
      $sLog.= $nCount.' participants were notified that ESA2 was available.<br /><br />';

      if($day == 'Sunday')
      {
        $sLog.= 'Send email after session ...<br />';
        $nCount = $this->_sendAfterSessionEmails();
        $sLog.= $nCount.' students were notified the course is finished.<br /><br />';
      }


      //log execution date and last report
      $oSetting->setSystemSettings('globus_daily_cron', $sToday);
      $oSetting->setSystemSettings('globus_daily_cron_log', $sLog);

      $oMail = CDependency::getComponentByName('mail');
      $oMail->createNewEmail();
      $oMail->setFrom(CONST_PHPMAILER_EMAIL, CONST_PHPMAILER_DEFAULT_FROM);
      $oMail->addRecipient('admin@globusjapan.com');
      $oMail->addBccRecipient(CONST_DEV_EMAIL);
      $oMail->send('Globus online training - notifications log', $sLog);
    }
  }
  ...
  private function _sendAfterSessionEmails()
  {
    ...
    foreach ($aEmails as $aEmail)
    {
      if(isset($aEmail['recipients']))
      {
        $oMail->createNewEmail();
        $oMail->setFrom(CONST_PHPMAILER_EMAIL, CONST_PHPMAILER_DEFAULT_FROM);
        $oMail->addRecipient($aEmail['recipients']);
        $oMail->addBccRecipient('admin@globusjapan.com', 'admin@globusjapan.com');
        $oMail->addBccRecipient(CONST_DEV_EMAIL, CONST_DEV_EMAIL);
  ...
  private function _notifyStudents()
  {
    ...
    foreach($aStudents as $nStudentPk => $asContent)
    {
      ...
      $sContent = 'Dear '.$aUser['firstname'].' '.$aUser['lastname'].','. $oHTML->getCarriageReturn(2);
      $sContent.= 'You have work to do on Globus Online Coaching Platform!'. $oHTML->getCarriageReturn(2);
      $sContent.= 'The following tests and/or ESA are close to reach the deadline:'. $oHTML->getCarriageReturn(1);
      $sContent.= $sHeader.implode('', array_unique($asContent)). $oHTML->getCarriageReturn(2);
      $sContent.= 'Please reply as soon as you can.'. $oHTML->getCarriageReturn(2);
  ...
  private function _notifyEsa2Available()
  {
    ...
    foreach($asNotification as $asData)
    {
      ...
      $sContent = 'Dear '.$asData['firstname'].' '.$asData['lastname'].','.$oHTML->getCarriageReturn(2);
...
事後アセスメント、 "ESA 2 (Email Simulation Assessment)"、に進む事が可能になりました。個人ページにログインし、提出期日を確認して下さい。設定された期日までにESA2を終わらせ、提出して下さい。';
  ...
  private function _displayStatsAverage($pnCompanyPk=0, $pnGroupPk=0, $pnEsa = 1)
  {
    ...
    $sHTML .= $oHTML->getCarriageReturn(1);
  ...
  private function _displayStudentEsa($pnAnswerPk, $pbIsStudentView = false)
  {
    ...
    $sHTML .= $oHTML->getBlocStart('esaReport');
      ...
      $sHTML.= $oHTML->getBlocStart('leftCol');
        ...
        $sHTML.= $oHTML->getBlocStart('skillSets', array('esa2' => (int)$bIsEsa2));
  ...
  private function _displayStudentTest($pnAnswerPk, $pbIsStudentView = false)
  {
    ...
    if($bAlert)
      $sHTML .= $oHTML->getErrorMessage('Before replying a new assignment, please read the comments of your trainer on this page and click on \'I read all comments\' ').$oHTML->getCarriageReturn(1);
  ...
  private function _notifyStudent($pnAnswerFk)
  {
    ...
    $sContent = 'Dear '.$aData['firstname'].' '.$aData['lastname'].','.$oHTML->getCarriageReturn(2);
    $sContent .= 'You receveid a correction from a trainer on Globus Online Coaching for the following test:'.$oHTML->getCarriageReturn(1);
    $sContent .= $sType.$aTest['rank'].': '.$aTest['name'].$oHTML->getCarriageReturn(2);
    $sContent .= 'Please '.$oHTML->getLink('check it out', $sUrl).' as soon as you can.'.$oHTML->getCarriageReturn(1);
