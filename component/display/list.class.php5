...
class CList
{
  ...
  {
    if(!assert('is_array($pasJavascript)'))
      return false;
  ...
  public function getDisplay()
  {
    ...
    foreach($this->casListData as $asRowData)
    {
      $sTbody.= '<tr>';
      ...
      foreach($asRowData['data'] as $sData)
      {
        $sTbody.= '<td>';
      ...
      if(!empty($asRowData['action']))
      {
        ...
        foreach($this->casLineAction as $asAction)
        {
    ...
    if($this->csHasHeader)
    {
      ...
      $nCount = 0;
      foreach($this->casColumns as $colData)
      {
