...
class CJobboarduserEx extends CJobboarduser
{
  ...
  */
  private function _getManageJobs($psType = '')
  {
    ...
    if($nTotal> 0)
    {
      ...
      if(!empty($asRecords))
      {
        ...
        $sQuery.= ' LEFT JOIN addressbook_industry as ind ON (ind.addressbook_industrypk = pos1.industryfk) ';
  ...
  private function _getSavePosition($pnPositionPk)
  {
    ...
    if(!empty($sCompany))
    {
      ...
      $sQuery = 'INSERT INTO addressbook_company (company_name, status) VALUES ('.$oDB->dbEscapeString($sCompany).',1)';
    ...
    if(!isJapanese($sPositionDesc))
    {
      foreach($asMetaKeywords as $sKeyword)
      {
        ...
        if($nKWLength > 2)
        {
          ...
          {
            assert('false; /* error parsing content for keywords ['.$sKeyword.']*/');
          }
        }
      }
    }
  ...
  private function _getJobEditForm($pnPk)
  {
    ...
    $sQuery.= ' LEFT JOIN addressbook_company as cp ON (cp.addressbook_companypk = companyfk AND cp.status > 0) ';
    ...
    foreach($asIndustries as $nValue => $avType)
    {
      ...
      if($nIndustry == $avType['addressbook_industrypk'])
      {
        $oForm->addOption('industry', array('value' => $avType['addressbook_industrypk'], 'label' => $sLabel, 'selected'=>'selected', 'style' => $sStyle));

        if((int)$avType['status'] == 2)
          $nParentIndus = $avType['parentfk'];
      }
      else
        $oForm->addOption('industry', array('value' => $avType['addressbook_industrypk'], 'label' => $sLabel, 'style' => $sStyle));
     }
  ...
  private function _getCompanySelector()
  {
    ...
    $sQuery = 'SELECT * FROM addressbook_company WHERE status = 1 AND lower(company_name) LIKE '.$oDB->dbEscapeString('%'.strtolower($sSearch).'%').' ORDER BY company_name desc';
    $oDbResult = $oDB->ExecuteQuery($sQuery);
    $bRead = $oDbResult->readFirst();
    if(!$bRead)
      return json_encode(array());
  ...
  */
  private function _getRemoveJobs($pnPositionPk)
  {
   ...
   private function _getCompanySend()
   {
     ...
     $sQuery = 'SELECT cp.company_name AS company_name,count(pos.positionpk) AS nCount FROM addressbook_company AS cp INNER JOIN position AS pos WHERE cp.addressbook_companypk = pos.companyfk AND cp.status != 0 and pos.visibility != 0 GROUP BY company_name ORDER BY company_name ASC';
     $oDbResult = $oDB->ExecuteQuery($sQuery);
     $bRead = $oDbResult->readFirst();
  ...
  */
  ...
  private function _addCompany($pnCompanyPk, $psCompanyName)
  {
    ...
    $sQuery = 'INSERT INTO addressbook_company (addressbook_companypk, company_name, status) ';
