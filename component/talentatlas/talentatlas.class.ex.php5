...
class CTalentatlasEx extends CTalentatlas
{
  ...
  private function _getMailFriendSave()
  {
    ...
    $sQuery = 'SELECT pos.*,cp.company_name,ind.name AS industry_name FROM position AS pos LEFT JOIN addressbook_company ';
    $sQuery.= ' AS cp ON cp.addressbook_companypk = pos.companyfk LEFT JOIN addressbook_industry AS ind ON pos.addressbook_industryfk = ind.industrypk WHERE pos.visibility <> 0 and pos.positionpk = '.$nPositionPk.'';
  ...
  private function _getSearchMessage($pnNbResult = 0)
  {
   ...
   private function _getJobSearchResult($psSearchId = '')
   {
    ...
    if(!empty($nIndustryPk))
      $sQuery.= ' INNER JOIN addressbook_industry AS ind ON (ind.addressbook_industrypk = pos.industryfk AND ind.addressbook_industrypk = '.$nIndustryPk.')';
    else
      $sQuery.= ' LEFT JOIN addressbook_industry AS ind ON (ind.addressbook_industrypk = pos.industryfk)';

    if(!empty($nCompanyPk))
      $sQuery.= ' INNER JOIN addressbook_company AS cp ON (cp.addressbook_companypk = pos.companyfk AND cp.addressbook_companypk = '.$nCompanyPk.') ';
    else
      $sQuery.= ' LEFT JOIN addressbook_company AS cp ON (cp.addressbook_companypk = pos.companyfk) ';
  ...
  private function _getJobResultList($pavResult, $psSearchId = '', $pbSearchFormOpen = false)
  {
    ...
    $oPage = CDependency::getCpPage();
    $oPager = CDependency::getComponentByName('pager');
    $oPage->addJsFile($this->getResourcePath().'js/talentatlas.js');
  ...
  private function _getSqlJobSearch()
  {
    ...
    if(!empty($sIndustry))
    {
      ...
      $asWhereSql[] = ' ind.addressbook_industrypk in ('.implode(',', $asIndustry).')';
    }
  ...
  private function _getHomePageCentre()
  {
    ...
    $oPage->addJsFile($this->getResourcePath().'js/jquery.anythingslider.js');
    ...
    $sHTML = $oHTML->getBlocStart('',array('class'=>'jobCentreSection'));
     ...
     $sHTML.= $oHTML->getBlocEnd();
      ...
      $sQuery = 'SELECT pos.*, IF(pos.company_label IS NOT NULL && pos.company_label <> "", pos.company_label, cp.company_name) as company_name  FROM position as pos LEFT JOIN addressbook_company as cp on pos.companyfk = cp.addressbook_companypk where pos.parentfk != 0 AND pos.visibility <> 0 AND pos.lang = "'.$sLang.'" ORDER BY positionpk DESC LIMIT 10 ';
      $oDbResult = $oDB->ExecuteQuery($sQuery);
      $bRead = $oDbResult->readFirst();
      if($bRead)
      {
  ...
  private function _getHomePageLeftList()
  {
    ...
    $sQuery = 'SELECT ind.name AS industry_name,count(pos.positionpk) AS nCount, addressbook_industrypk  FROM addressbook_industry AS ind INNER JOIN position AS pos WHERE ind.addressbook_industrypk = pos.industryfk AND ind.status != 0 and pos.visibility != 0 and pos.lang = "'.$sLang.'" GROUP BY ind.addressbook_industrypk ORDER BY industry_name  ASC LIMIT 15';
    $oDbResult = $oDB->ExecuteQuery($sQuery);
    $bRead = $oDbResult->readFirst();
       ...
       if(isset($this->casText[$oDbResult->getFieldValue('industry_name')]))
        ...
        $sHTML.= $oHTML->getBlocStart('',array('class'=>'industryFeatured'));
         $sHTML.= $oHTML->getBlocStart('', array('class' => 'leftFeaturedName'));
          $sHTML.= $oHTML->getLink($sIndustryName,'javascript:;',array('onclick'=>'submitForm(this);','class'=>'middleTitle','industrypk'=>$oDbResult->getFieldValue('addressbook_industrypk'),'industryname'=>$oDbResult->getFieldValue('industry_name')));
           $sHTML.= $oHTML->getBlocEnd();
    ...
    $sQuery = 'SELECT cp.company_name AS company_name,count(pos.positionpk) AS nCount, addressbook_companypk  FROM addressbook_company AS cp INNER JOIN position AS pos WHERE cp.addressbook_companypk = pos.companyfk AND cp.status != 0 and pos.visibility != 0 AND pos.lang = "'.$sLang.'" AND company_name != "" GROUP BY company_name ORDER BY nCount DESC LIMIT 10';
    $oDbResult = $oDB->ExecuteQuery($sQuery);
    $bRead = $oDbResult->readFirst();
    ...
    if($bRead)
    {
     while($bRead)
     {
        $sHTML.= $oHTML->getBlocStart('',array('class'=>'industryFeatured'));
         $sHTML.= $oHTML->getBlocStart('', array('class' => 'leftFeaturedName'));
          $sHTML.= $oHTML->getLink($this->_removeJapanese($oDbResult->getFieldValue('company_name')),'javascript:;',array('onclick'=>'submitForm(this);','class'=>'middleTitle','companypk'=>$oDbResult->getFieldValue('addressbook_companypk'),'companyname'=>$oDbResult->getFieldValue('company_name')));
           $sHTML.= $oHTML->getBlocEnd();
            $sHTML.= $oHTML->getBlocStart('',array('class'=>'leftFeaturedNumber'));
           $sHTML.= $oHTML->getText(' '.$oDbResult->getFieldValue('nCount'),array('style'=>'color:#D4662A;'));
         $sHTML.= $oHTML->getBlocEnd();
  ...
  private function _getJobSearchForm($pbIsAjaxForm = false)
  {
    ...
    {
      if(in_array($vName['addressbook_industrypk'],$asIndustrySelected))
        $oForm->addOption('industry_tree', array('title' => ''.$this->casText[$vName['name']].'','id' => $vName['addressbook_industrypk'], 'parent' => $vName['parentfk'],'checked'=>'checked'));
      else
        $oForm->addOption('industry_tree', array('title' => ''.$this->casText[$vName['name']].'','id' => $vName['addressbook_industrypk'], 'parent' => $vName['parentfk']));
    }
   ...
   */
   ...
   private function _getJobDetailInformation($pnPk)
   {
     ...
     $sQuery = 'SELECT pos.*, ind.*, IF(pos.company_label IS NOT NULL && pos.company_label <> "", pos.company_label, cp.company_name) as company_name, ind.name AS industry_name FROM position AS pos LEFT JOIN addressbook_company ';
     $sQuery.= ' AS cp ON cp.addressbook_companypk = pos.companyfk LEFT JOIN addressbook_industry AS ind ON pos.industryfk = ind.addressbook_industrypk WHERE pos.visibility <> 0 and pos.positionpk = '.$pnPk.'';
     $oDbResult = $oDB->ExecuteQuery($sQuery);
     $bRead = $oDbResult->readFirst();
  ...
  public function getJobApplyForm($pnPk)
  {
   if(!assert('is_integer($pnPk) && !empty($pnPk)'))
     ...
     if(!empty($pnPk))
     {
      ...
      $oForm->setFormParams('', false, array('submitLabel' => $this->casText['TALENT_APPLY_NOW'],'action' => $sURL, 'onBeforeSubmit'=>'if(!$(\'#agree_terms_id:checked\').val()){ alert(\'Please accept the terms and conditions.\'); return false; };'));
  ...
  private function _getResumeSave($pnPositionPk)
  {
    ...
    $sQuery = 'SELECT pos.*,cp.company_name,ind.name AS industry_name FROM position AS pos LEFT JOIN addressbook_company ';
    $sQuery.= ' AS cp ON cp.addressbook_companypk = pos.companyfk LEFT JOIN addressbook_industry AS ind ON pos.industryfk = ind.addressbook_industrypk WHERE pos.visibility <> 0 and pos.positionpk = '.$pnPositionPk.'';
    ...
    */
    public function getIndustries($pnIndustryPk = 0, $pbParentFirst = false, $pbInUseOnly = false, $pbNonTranslated = false)
    {
      ...
      if($pbInUseOnly)
      {
        $sExtraJoin = ' LEFT JOIN position as pos ON (pos.industryfk = addressbook_industrypk AND pos.status = 1) ';
        $sExtraWhere = ' AND (ind.parentfk = 0 OR pos.industryfk IS NOT NULL) ';
        $sExtraGroup = ' GROUP BY ind.addressbook_industrypk ';
      }
      ...
      if(empty($pnIndustryPk))
      {
        ...
        $sQuery = 'SELECT DISTINCT ind.* FROM addressbook_industry as ind '.$sExtraJoin.' WHERE  '.$sStatus.' '.$sExtraWhere.' '.$sExtraGroup.' ORDER BY '.$sExtraOrder.' ind.name ASC ';
        $oResult = $oDB->ExecuteQuery($sQuery);
        $bRead = $oResult->readFirst();
        ...
        while($bRead)
        {
          $asIndustries[$oResult->getFieldValue('addressbook_industrypk')] = $oResult->getData();
          $bRead = $oResult->readNext();
        }
        return $asIndustries;
      }

      $sQuery = 'SELECT DISTINCT ind.* FROM addressbook_industry as ind '.$sExtraJoin.' WHERE addressbook_industrypk = '.$pnIndustryPk.' '.$sExtraWhere.' '.$sExtraGroup;
      $oResult = $oDB->ExecuteQuery($sQuery);
      $bRead = $oResult->readFirst();
