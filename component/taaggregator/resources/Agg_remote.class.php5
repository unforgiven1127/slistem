   ...
   protected function checkLanguageLevel($psString,$psType)
   {
     switch($psType)
      ...
      protected function _getcheckIndustry($psIndustryName, $pnParentId = 0)
      {
        ...
        $sQuery = 'SELECT * FROM addressbook_industry WHERE name like "%'.$psIndustryName.'"';
        $oResult = $oDB->ExecuteQuery($sQuery);
        $bRead = $oResult->readFirst();
        ...
        if($bRead)
        {
          ...
          return $oResult->getFieldValue('addressbook_industrypk', CONST_PHP_VARTYPE_INT);
        }
        ...
        $sQuery = 'INSERT INTO  addressbook_industry (`name`, `status`, `parentfk`) VALUES('.$oDB->dbEscapeString($psIndustryName).', 2, '.$nParentfk.')';
        $oResult = $oDB->ExecuteQuery($sQuery);
      ...
      protected function _getcheckCompany($psCompanyName)
      {
        ...
        {
          $sQuery = 'SELECT * FROM addressbook_company WHERE company_name = "'.$psCompanyName.'"';
          $oResult = $oDB->ExecuteQuery($sQuery);
          $bRead = $oResult->readFirst();
          ...
          {
            $sQuery = 'INSERT INTO  addressbook_company (`company_name`,`status`) VALUES("'.$psCompanyName.'",1)';
            $oResult = $oDB->ExecuteQuery($sQuery);
