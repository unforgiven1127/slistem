...
class CZCalendar extends CZimbraEx
{
  ...
  private $casExcludedUser = array('siraa_lviv', 'afujiwara', 'mbertumen', 'amaher', 'bhersey');
 ...
 protected function _getSaveAppointment()
 {
   ...
   //create the appointement in the user calendar and in the local database
    ...
    if(in_array($asOrganizerData['login_groupfk'], array(1,3)))
      $bSalesTeam = true;
    else
      $bSalesTeam = false;
    ...
    if($bInAjax)
    {
      ...
      $sJavascript.= ' setTimeout("goPopup.removeActive();", 2000); ';
    ...
    */
    public function getCalendarForm($pbInAjax = false)
    {
      ...
      $oForm->setFormDisplayParams(array('noCancelButton' => 1, 'columns' => 2, 'noSubmitButton' => 1));
      ...
      $oForm->addField('input', 'date_start', array('type' => 'datetime', 'class' => 'dateStartField', 'label'=>'Start Time', 'value' => date('Y-m-d H:', strtotime('+1 hour')).$nMin, 'monthNum' => 2));
      ...
      $oForm->addField('select', 'attendees[]', array('label' => 'Attendees', 'class' => 'attendeesField', 'multiple' => 'multiple', 'allNoneLink' => 1));
      ...
      $sJavascript.= ' var sUser = $(".attendeesField:not(:visible)").val(); ';
      $sJavascript.= ' var sDate = $(".dateStartField").val(); ';
      $sJavascript.= ' if(!sDate){ goPopup.setErrorMessage(\'Starting date is empty.\'); return false; }';
  ...
  */
  ...
  public function getsubCreateAppointment($pasCreate)
  {
    ...
    $sId = $oXml->addNode(array('name'=>'CreateAppointmentRequest','@attributes' =>array('comp'=>'"0"','xmlns'=>'"urn:zimbraMail"')));
    $sId1 = $oXml->addChildNode($sId, array('name'=>'m','@attributes' => array('l'=>'"'.$pasCreate['calendar'].'"')));
    $oXml->addChildNode($sId1, array('name'=>'su','@value'=>'<![CDATA['.$pasCreate['title'].']]>'));
    $oXml->addChildNode($sId1, array('name'=>'e','@attributes' => array('a'=>'"'.$pasCreate['organizer']['email'].'"','d'=>'"'.$pasCreate['organizer']['name'].'"','p'=>'"'.$pasCreate['organizer']['name'].'"','t'=>'"f"')));
    ...
    $sDescription = str_ireplace(array('<br/>', '<br/>', '</p>', "\r"), "\n", $pasCreate['notes']);
    $sDescription = strip_tags($sDescription);
    $oXml->addChildNode($sId3, array('name' => 'desc','@value'=>'<![CDATA['.$sDescription.']]>'));
    $oXml->addChildNode($sId3, array('name' => 'descHtml', '@value'=>'<![CDATA['.$pasCreate['notes'].']]>'));
    $oXml->addChildNode($sId3, array('name' => 'or','@attributes' => array('a'=>'"'.$pasCreate['organizer']['email'].'"','d'=>'"'.$pasCreate['organizer']['name'].'"','sentBy'=>'"'.$pasCreate['organizer']['email'].'"','dir'=>'"in:inbox"','lang'=>'"en-US"')));
    ...
    */
    private function _refreshLocalCalendars($pasCalendarEvent)
    {
      ...
      //fetch alternative email addresses to allow events from other calendars to be displayed
      $sQuery = 'SELECT * FROM zimbra_external_account AS zext ';
      $oDbResult = $oDB->ExecuteQuery($sQuery);
      $bRead = $oDbResult->readFirst();
      while($bRead)
      {
        $asUserEmail[$oDbResult->getFieldvalue('email')] = $oDbResult->getFieldvalue('loginfk');
        $bRead = $oDbResult->readNext();
      }
          ...
          if(count($asEventData['evAttendee']) > 1)
          {
            foreach($asEventData['evAttendee'] as $sUserIdentifier => $nTeamfk)
            {
              ...
              else
              {
                ...
                if(isset($asUserEmail[$sUserIdentifier]))
                {
                ...
                {
                  //since it mopves a lot and the table is not updated
                  //let's try to look for alternatve  @tokyoweekender or @slate.co.jp @gmail
                  $asEmail = explode('@', $sUserIdentifier);
                  if(count($asEmail) == 2)
                  {
                    $asAltEmail = array($asEmail[0].'@bulbouscell.com', $asEmail[0].'tokyoweekender.com', $asEmail[0].'@slate.co.jp', $asEmail[0].'@gmail.com');
                    foreach($asAltEmail as $sEmail)
                    {
                      echo 'user unknown, looking for alternatives '.$sEmail.'<br />';
                      if(isset($asUserEmail[$sEmail]))
                      {
                        $nUserfk = (int)$asUserEmail[$sEmail];
                        $sEmail = $sUserIdentifier;
                        break;
                      }
                    }
              ...
              if(!empty($nUserfk) && $nUserfk != $nOwnerfk)
              {
  ...
  protected function syncWithZimbra($pbRefreshShared = true, $pbUpdateLocal = false)
  {
    ...
    echo '<hr/><hr/><hr/>'.count($asCalendarData).' events fetched from zombra<hr /><hr/><hr/>';
  ...
  */
  private function _getEventsFromZimbra()
  {
    ...
    foreach($asUsers as $skey => $asUser)
    {
      ...
      if(in_array($sUserId, $this->casExcludedUser))
      {
        echo '<hr />'.__LINE__.' '.$sUserId.' is a blacklisted user... ';
      }
      else
      {
          ...
          if(!empty($asAppointments))
          {
            foreach($asAppointments as $skey => $asData)
            {
              if(isset($asData['INVID']) && !empty($asData['INVID']))
              {
                ...
                {
                  assert('false; // Error '.__LINE__.' - weidly formated or empty event ('.$sInvId.') ');
                  dump($asData);
                ...
                else
                {
                  ...
                  if(isset($asAppointmentData['INV']['COMP']['AT']) && !empty($asAppointmentData['INV']['COMP']['AT']) && is_array($asAppointmentData['INV']['COMP']['AT']))
                  {
                    ...
                    if(isset($asAppointmentData['INV']['COMP']['AT'][0]))
                    {
                      foreach($asAppointmentData['INV']['COMP']['AT'] as $asAtt)
                      {
                        ...
                        {
                          //dump($asAppointmentData);
                          //assert('false; // don\'t know this attendee email address ['.$asAtt['A'].']');
                          $asAttendee[$asAtt['A']] = 0;
                        }
                        else
                        {
                          /*dump("goooooo 1: ZIMBRA asAppointmentData['INV']['COMP']['AT']");
                          dump($asAppointmentData['INV']['COMP']['AT']);*/

                          $nUserKey = $asUserEmail[$asAtt['A']];
                          if(isset($asUsers[$nUserKey]))
                          {
                            //$asAttendee[$asUsers[$nUserKey]['email']] = $asUser['login_groupfk'];
                            $asAttendee[$asAtt['A']] = $asUser['login_groupfk'];
                          }
                          else
                          {
                            assert('false; // don\'t know this attendee email address ['.$asAtt['A'].']');
                      ...
                      $asAppointmentData['INV']['COMP']['AT']['A'] = $asUser['login_groupfk'];
                  }
                  else
                    $asAttendee = array($asUser['email'] => $asUser['login_groupfk']);
                  ...
                  else
                  {
                    ...
                    elseif(isset($asAppointmentData['INV']['COMP']['DESC']['DATA']))
                    {
                      $sDescription = $asAppointmentData['INV']['COMP']['DESC']['DATA'];
                    }
                    elseif(isset($asAppointmentData['INV']['COMP']['ALARM']['DESC']['DATA']))
                    {
                      $sDescription = $asAppointmentData['INV']['COMP']['ALARM']['DESC']['DATA'];
                    }

                    $sOwner = $oLogin->getUserNameFromData($asUsers[$asUser['loginpk']], true);
                    ...
                    $asCalendarData[$sInvId.'_'.$asAppointmentData['INV']['COMP']['S']['D'].'_'.$asAppointmentData['INV']['COMP']['E']['D']] =
                            array('evID' => $sInvId, 'evTitle' => $asAppointmentData['INV']['COMP']['NAME'], 'evDesc' => $sDescription,
                                  ...
                                  'evOwner' => $sOwner, 'evOwnerGroup' => $asUsers[$asUser['loginpk']]['login_groupfk'], 'evOwnerEmail' => $asUsers[$asUser['loginpk']]['email'],
        ...
        else
        {
          ...
          if(!in_array($sUserId, $this->casExcludedUser))
          {
            assert('false; /*can not log to bcm calendar acount as ['.$sUserId.'] errors:'.$this->getErrors(true).' */');
          }
  ...
  private function _refreshSharedCalendars($pasCalendarEvent)
  {
    ...
    if(getValue('wipeOnly', 0))
    {
      ...
      dump('Asked for a WIPE ? DONE !! O_o ');
      return true;
    }
    ...
    {
      assert('false; //can not log to main [bcm] calendar account. Errors: '.$this->getErrors(true));
      echo $this->getErrors(true);
      return false;
    }
    ...
    foreach($pasCalendarEvent as $asData)
    {
      if(!empty($asData))
      {
        if(empty($asData['evAttendee']) && empty($asData['evOwner']['name']))
        {
          dump('No attendees and no owner  ????? Hen dayo');
          dump($asData);
        }

        //check if there's an owner. If not, set a default one
        // we add it in the attendees so it's displayed in the correct calendar (teamfk = ownerGroup)
        if(!empty($asData['evOwner']))
        {
          $bOrganizer = true;
          $asBCMOrganizer = array('email' => $asData['evOwnerEmail'], 'name' => $asData['evOwner']);
          $asData['evAttendee'][$asData['evOwnerEmail']] = $asData['evOwnerGroup'];
        }
        else
        {
          $bOrganizer = false;
          $asBCMOrganizer = array('email' => 'bcm@bulbouscell.com', 'name' => ' - unknown - ');
          $asData['evAttendee']['bcm@bulbouscell.com'] = 0;
        }

        //check all attendee and create short names to display in title
        //dump($asData['evAttendee']);
        $asAttendeeShortName = array();
        $asAttendeeName = array();
        $nCount = 0;
        foreach($asData['evAttendee'] as $sEmail => $nKey)
        {

          $asParts = explode('@', $sEmail);
          $sShort = substr($asParts[0], 0, 3);
          $asAttendeeShortName[$sShort] = $sShort;
          $asAttendeeName[$sEmail] = $sEmail;

          //If there s no organizer, we set the first attendee found as organizer
          if(!$bOrganizer && $nCount == 0)
            $asBCMOrganizer = array('email' => $sEmail, 'name' => $asParts[0]);

          $nCount++;
        }

        if(empty($asAttendeeName))
          $asAttendeeName[$asBCMOrganizer['email']] = $asBCMOrganizer['email'];



        //remove duplicates attendee / owner
        $sShortOwner =  substr($asBCMOrganizer['email'], 0, 3);
        unset($asAttendeeShortName[$sShortOwner]);
        unset($asAttendeeName[$asBCMOrganizer['email']]);

        $sEventName = $sShortOwner;

        if(!empty($asAttendeeShortName))
          $sEventName.= '('.implode(', ', $asAttendeeShortName).')';

        $sEventName.= ': '.$asData['evTitle'];


        if(!empty($asData['evDesc']))
        {
          $sEventNote = '<div style="background-color: #f0f0f0; padding: 20px 10px;">'.$asData['evDesc'].'</div>';
        }
        else
        {
          $sEventNote = '<div style="background-color: #f0f0f0; padding: 20px 10px;"><small><em> - no description available - </em></small></div>';
        }

        if(!empty($asAttendeeName))
          $sEventNote.= '<br/><br/>With: '.implode(', ', $asAttendeeName);

        $sEventNote.= '<br/><br/><br/><br/><small><em>BCM shared calendar last updated on '.$sUpdateDate.'</em></small>';
  ...
  */
  ...
  public function getHomepageUserCalendar()
  {
    ...
    $sQuery = 'SELECT *, DATE(starttime) as eventDate, DATE_FORMAT(starttime, "%V") as eventWeek,
      DATE_FORMAT(starttime, "%H:%i") as eventStart,
      DATE_FORMAT(endtime, "%H:%i") as eventEnd,
      DATE_FORMAT(endtime, "%Y:%m%d") as startDate

      FROM zimbra_cal AS zcal
      LEFT JOIN zimbra_attendees AS zatnd ON (zatnd.zcalfk = zcal.zimbra_calpk AND zatnd.loginfk = '.$nLoginPk.')

      WHERE (zcal.creatorfk = '.$nLoginPk.' OR zatnd.loginfk = '.$nLoginPk.')
      AND starttime >= "'.date('Y-m-d').'" AND starttime <  "'.date('Y-m-d H:i:s',strtotime('+14 days')).'"
      GROUP BY invId, starttime ORDER BY starttime ASC  LIMIT 0, 10 ';
    //echo $sQuery;
    ...
    if(!$bRead)
    {
      ...
      $sHTML.= $oDisplay->getText('No events to display.', array('style' => 'margin: 10px 0; display: block; '));
    ...
    while($bRead)
    {
      ...
      $asCalDate[$oDbResult->getFieldValue('startDate')][] = $oDbResult->getData();
    ...
    /*//new version
    $asTile = array();
    foreach($asCalDate as $sDate => $asEntry)
    {
      $nEventOfTheDay = count($asEntry);

      if($nEventOfTheDay == 1)
        $sClass = 'calendar_single';
      elseif($nEntries > 1)
        $sClass = 'calendar_half';
      else
        $sClass = 'calendar_quarter';

      $sTile = $oDisplay->getBlocStart('', array('class' => $sClass));
      $sTile.= $oDisplay->getBlocEnd();
      $sTile.= $oDisplay->getBlocEnd();
  ...
  protected function _getViewCalendar()
  {
    ...
    //$oPage->addJsFile($this->getResourcePath().'fullcalendar/fullcalendar.min.js');
    $oPage->addJsFile($this->getResourcePath().'fullcalendar/fullcalendar.js');
    ...
    $sHTML.= $oDisplay->getBlocStart('mainCalendareContainer', array('style' => 'width: 100%; min-height: 650px; position: relative;'));
      ...
      $oForm->setFormParams('', false, array('action' => 'javascript:;'));
      $oForm->setFormDisplayParams(array('noCancelButton' => 1, 'columns' => 2, 'noSubmitButton' => 1));
      ...
      foreach($asUsers as $asUserData)
      {
        if($nPreSelected == $asUserData['loginpk'])
          $oForm->addOption('calUsers', array('label' => $asUserData['firstname'].' '.$asUserData['lastname'], 'value' => $asUserData['loginpk'], 'team' => $asUserData['login_groupfk'], 'selected' => 'selected'));
        else
          $oForm->addOption('calUsers', array('label' => $asUserData['firstname'].' '.$asUserData['lastname'], 'value' => $asUserData['loginpk'], 'team' => $asUserData['login_groupfk']));
      }
      ...
      $aGroups = $oLogin->getUserGroup(0, true);

      foreach ($aGroups as $aGroup)
      {
        $sHTML.= $oDisplay->getText(' - ');
        $sHTML.= $oDisplay->getLink($aGroup['title'], 'javascript:;', array('onclick' => $sJavascript.'$(\'#calUsersId\').children(\'option[team=\''.$aGroup['login_grouppk'].'\'],option[team=\''.$aGroup['login_grouppk'].'\']\').attr(\'selected\', \'selected\').end().change();'));
      }
      $sHTML.= $oDisplay->getBlocEnd();
    ...
    $(document).ready(function()
    {
      ...
      $("#calendarViewContainer").fullCalendar(
      {
        ...
        {
          $(".fc-event-time", element).prepend(event.owner+" - ");

          //setting up the div content (will probably be truncated, so we ll add a tooltip next)
          if(event.attendees)
            $(".fc-event-title", element).html(event.attendees+" - "+event.title);
          else
            $(".fc-event-title", element).html(event.title);

          $(".fc-event-title", element).append("<br /><br />"+event.description);
          $(".fc-event-title", element).css("background-color", event.eventColor);
          $(element).css("background-color", event.eventColor);


          //generate the string for the tooltip
          var sTooltip = "<strong><em>"+event.eventTime+"</em></strong><br/><br/>"+event.owner+" - <span class=\'eventTitle\'>"+event.title+"</span><br/><br/>";

          if(event.attendees)
            sTooltip+= "<strong><em>Attending:</em></strong> "+event.attendees+"<br/><br/>";

          if(event.description)
           sTooltip += "<strong><em>Detail:</em></strong> "+event.description+"<br/><br/>";

          $(".fc-event-inner", element).attr("title", sTooltip).tooltip({content: sTooltip});
        }
      });
    });
    </script>';
  ...
  protected function _getLocalEventsForCalendar()
  {
    ...
    $sQuery = 'SELECT *, GROUP_CONCAT(DISTINCT(zcal.creatorfk)) as creators,
      GROUP_CONCAT(DISTINCT(zatnd.loginfk)) as attend,
      GROUP_CONCAT(DISTINCT(zatnd.email)) as attendees
      FROM zimbra_cal AS zcal ';
    ...
    while($bRead)
    {
      ...
      //Multiple ways of getting the attendees :/
      $sAttendees = '';
      $asAttendees = array();

      if($oDbResult->getFieldValue('creators'))
        $asAttendees = explode(',', $oDbResult->getFieldValue('creators'));

      if($oDbResult->getFieldValue('attend'))
        $asAttendees = array_merge($asAttendees, explode(',', $oDbResult->getFieldValue('attend')));

      if($oDbResult->getFieldValue('attendees'))
        $asAttendees = array_merge($asAttendees, explode(',', $oDbResult->getFieldValue('attendees')));

      if(isset($asEventData['attendees']))
        $asAttendees = array_merge($asAttendees, $asEventData['attendees']);

      $asAttendees = array_unique($asAttendees);

      foreach($asAttendees as $sKey => $sId)
      {
        //if the attendee is a pk, convert into BCM user. Else, keep the external email address as it is
        if(is_numeric($sId))
        {
      ...
      $sDescription = strip_tags($sDescription);
      $sOwner = $oLogin->getUserNameFromData($asUsers[$nCreatorFk], true);
      $sTitle = $asEventData['title'];
      ...
      if(isset($asEvents[$sEventId]))
      {
        ...
        if(!empty($asAttendees))
        {
          if(!empty($asEvents[$sEventId]['attendees']))
            $asAttendees = array_merge($asAttendees, (array)$asEvents[$sEventId]['attendees']);
        }
      }
      else
      {
        if(!empty($asAttendees))
          $sAttendees = implode(', ', $asAttendees);

        if(!isset($this->casColor[$nCreatorFk]))
          $this->casColor[$nCreatorFk] = '#eee';
